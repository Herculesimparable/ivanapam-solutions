#!/usr/bin/env python3
"""Processa o logo Gelsadas para a secção de parceiros."""
...
SRC_CANDIDATES = [
    SOURCE_DIR / "gelsadas-logo.png",
    ROOT / "Ivanapammm" / "gelsadas-logo.png",
    Path(
        ...
        r"images_Gelsadas-8fb60e6f-0fb1-4822-b5aa-9971aae5c54f.png"
    ),
]
...
def resolve_source() -> Path:
    for path in SRC_CANDIDATES:
        if path.exists():
            return path
    raise SystemExit(
        "Fonte Gelsadas não encontrada. Coloque gelsadas-logo.png em assets/generated/"
    )


def normalize_background(img: Image.Image) -> Image.Image:
    """Fundo cinza claro -> branco; preserva cores do logo."""
...
def export_variants(logo: Image.Image) -> None:
    ...
    card.save(OUT_DIR / "gelsadas.webp", "WEBP", quality=92, method=6)
    card.save(OUT_DIR / "gelsadas.jpg", "JPEG", quality=91)
    hd.save(OUT_DIR / "gelsadas-hd.webp", "WEBP", quality=93, method=6)
    hd.save(OUT_DIR / "gelsadas-hd.jpg", "JPEG", quality=92)
...
def sync_site() -> None:
    ...
    for p in site["partners"]:
        if p["id"] == "gelsadas":
            p["logo"] = "assets/img/partners/gelsadas.webp"
            p["logoJpg"] = "assets/img/partners-4k/gelsadas.jpg"
            p["logoSvg"] = "assets/img/partners/gelsadas.webp"
...
def main() -> None:
    ...
    persistent = SOURCE_DIR / "gelsadas-logo.png"
    if src != persistent:
